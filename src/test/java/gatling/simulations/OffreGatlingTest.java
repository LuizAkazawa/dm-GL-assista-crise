...
public class OffreGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all offres").get("/api/offres").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new offre")
                        .post("/api/offres")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" + "\"description\": null" + ", \"geolocalisation\": \"SAMPLE_TEXT\"" + ", \"isArchived\": null" + "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_offre_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created offre").get("${new_offre_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created offre").delete("${new_offre_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Offre entity").exec(scn);
